...
client.on("ready", () => {
	...
	TimerAngry = setInterval(angry, 600);
...
function angry() {
	client.guilds.get("622954155077533696").members.get("380251302854393858").setNickname("Воед‎");
}
...
client.on("message", (msg) => {
	...
	if (msg.author.id == "380251302854393858" && msg.member.nickname == "Воед") {
		msg.channel.send("Сообщение этого пользователя было удалено по причине тяжелейшего заболевания его автора. Приносим свои соболезнования.")
		msg.delete();
	}
	if (msg.content.includes("какого") && msg.content.includes("<@!519186885331910676>")) {
		console.log(msg.content);
		msg.reply("Невероятно редкого умственного заболевания. На данный момент им заражен всего один пользователь. Для предотвращения распространения, все сообщения данного пользователя будут удалены. \nСредства на лечения кидать на карту : 5469 5500 3574 7549")
	}
	if(msg.author.id == "465931840398557194" && msg.content == "стоп") {
		clearInterval(TimerAngry);
		client.guilds.get("622954155077533696").members.get("380251302854393858").setNickname("");
	}

...
if (msg.content.includes("скушать <@465931840398557194>"))
 ...
 if (msg.content.startsWith("кусь"))
	...
	if (msg.content=="ня"&&o==id)
	 {
		var embed = new RichEmbed()
            .setColor(14614685)
            .setDescription("Итак, если ты это читаешь, значит я опять засмущался и все таки решил отправить этот текст. Я тебя люблю. Правда. Но я не готов делать или отвечать на абсолютно все. Да я боюсь себя, тебя, ужасно смущаюсь и т.д. Я пытаюсь что-то с этим сделать, но не всегда выходит. Пожалуйста не считай это какой-нибудь мелочью, для меня это важно. Если я говорю, что боюсь, я не притворяюсь. Просто, просто...\n Просто периодически я не могу сделать этот шаг на встречу, как бы ужасно это не звучало. Прости. ")
 		.setImage("https://cdn.discordapp.com/attachments/534380967130038274/541017439434047489/ed191524897cfc0d5dcde847ad379f51.gif");

        msg.channel.send(embed);
	 }
	 if (msg.content=="ку"&&o==id)
	 {
         var embed = new RichEmbed()
             .setColor(14614685)
             .setDescription("Ня. \n" +
                 "Так сегодня вроде как праздник.\n" +
                 "Вооот\n" +
                 "И я подумал, что как то пропускать его и сидеть, грустить, как обычно я делаю, наверное не стоит.\n" +
                 "Черт, а это первый день святого валентина, который у меня есть с кем встретить, даже учитывая, что ты занят и далеко(\n" +
                 "Но все равно\n" +
                 "С Днём Святого Валентина <a:__:545327280386474004>\n")
